...
// --- CONSULTA ASÍNCRONA A get_destino.php ---
async function obtenerDestinoDesdeBD() {
    const urlParams = new URLSearchParams(window.location.search);
    const idDestino = urlParams.get('id') || 1; // ID 1 por defecto

    try {
        const respuesta = await fetch(`../destinos/get_destinos.php?id=${idDestino}`);
        if (!respuesta.ok) throw new Error('No se pudo obtener la información del destino');
        
        destinoActual = await respuesta.json();
...
function renderizarInterfaz(destino) {
    ...
    const totalBase = c.transporte + c.comida + c.entradas;
    const tablaCostos = document.querySelector('.table-costs tbody');
    if (tablaCostos) {
        tablaCostos.innerHTML = `
            <tr>
                <td>🚌 Transporte / Pasaje</td>
                <td>$${c.transporte.toFixed(2)}</td>
            </tr>
            <tr>
                <td>🍽️ Alimentación</td>
                <td>$${c.comida.toFixed(2)}</td>
            </tr>
            <tr>
                <td>🎟️ Entradas</td>
                <td>$${c.entradas.toFixed(2)}</td>
            </tr>
            <tr class="row-total">
...
function irAlPlanificador() {
    ...
    window.location.href = `planificador.html?destino=${encodeURIComponent(nombre)}&transporte=${c.transporte}&comida=${c.comida}&entradas=${c.entradas}`;
}